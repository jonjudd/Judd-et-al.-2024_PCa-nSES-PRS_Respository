...
summ_df %>%
  write_csv( file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Draft_Figures/Figure1_models_2024.03.21.csv")
...
# And now export these models.
summ_df %>%
  write_csv( file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Draft_Figures/Figure2_models_2024.03.21.csv")
...
table2 %>% 
  write_csv(file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Draft_Figures/Table2_models_2024.03.21.csv")
...
# And now export these models.
summ_df %>%
  write_csv( file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Draft_Figures/Figure3_models_2024.03.21.csv")
...
# And now export these models.
summ_df %>%
  write_csv( file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Draft_Figures/Figure4_models_2024.03.21.csv")
...
# And now export this table
table4 %>% 
  write_csv(file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Draft_Figures/Table4_models_2024.03.21.csv")
...
## Supp 3 - Healthcare Utilization

I will use this next section to start incorporating some of the models/questions I have about healthcare utilization. I will specificially make 2 sets of models
- Healthcare Utilization ~ nSES
...
for (j in ances){
  ...
  for (i in ses_factors){
    ...
    # Run models of interest
      formula = paste0("no_visits ~ Deprivation_z + age_assessment + PSA_Screening"
                       ) %>% as.formula()
...
# And now export these models.
summ_df %>%
  write_csv( file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Supp_Figures/Supp3a_models_2024.04.17.csv")


```

We want to use this #_visits results to make plots (NOT COMPLETE AT ALL)

```{r}

# Build main volcano plot
p = summ_df %>%
  ggplot(aes(y = fct_rev(ses))) +
  theme_classic() +
  theme(legend.position="top") +
...
summ_df2 = tibble() # For #_visits effects
...
for (j in ances){
  ...
  model_results2 = list() # This is for the #_visits effects
  ...
  # Save the models
  results_df = list_rbind(model_results, names_to = "ses") 
  results_df2 = list_rbind(model_results2, names_to = "ses") 
  ...
  # Repeat the same for the healthcare utilization effects
  summ_df2 = results_df2 %>% 
    mutate(ancestry = j) %>% 
    rbind(summ_df2, .) 
...
# And now export these models.
summ_df %>%
  write_csv( file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Supp_Figures/Supp3b_models_2024.04.17.csv") # Saving the effect of nSES

summ_df2 %>%
  write_csv( file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Supp_Figures/Supp3c_models_2024.04.17.csv") # Saving the effect of # of visits
```
