...
I also wanted to run this analyses to see if the deprivation association holds for other diseases. The first disease I will check is Colorectal Cancer. First thing I need to do is load the dataset similar to the prostate cancer dataset

I then need to run the Figure 2(Disease~nSES) analysis

```{r}
data_raw = fread("/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Composite_DataFiles/composite_data_crc_2024.02.21.csv")
...
# I also want to make sure to have new cancer variables that I can plug into my models. 
data_df = data_raw %>% 
  #First, I want a variable that counts any colorectal cancer as case and no cancer at all as the control. 
  # If no colorectal cancer, but other cancer, then that is NA
  mutate(colorectal_case = if_else(colorectal == 1, 1, 
                                 if_else(case == 1, NA, 0)
                                 )
         ) %>% 
  # I also want to include a measure of incident cancer
  mutate(colorectal_incident = if_else(colorectal == 1 & colorectal_incid_any == 0, NA,
                                     if_else(colorectal_incid_any == 0 & case == 0, 0, 
                                             if_else(colorectal_incid_any == 1, 1, NA))
                                     )
         ) %>% 
  # I also want prevalent cancer
  mutate(colorectal_prevalent = if_else(colorectal_incid_any == 1, NA,
                                      if_else(colorectal==0 & colorectal_incid_any==0 & case ==0, 0, 
                                              if_else(colorectal==1 & colorectal_incid_any==0, 1, NA))
                                      )
         ) %>% 
  # Make all of these factors
  mutate(colorectal_case = as.factor(colorectal_case),
         colorectal_incident = as.factor(colorectal_incident),
         colorectal_prevalent = as.factor(colorectal_prevalent)) %>% 
  # Remove non-NA's for screening PSA
  filter(!is.na(PSA_Screening)) %>% 
  # I want to remove non-UK assessment centers
  filter(Assessment_Centre %notin% c(11023, 11022, 11003, 11005, 11004)
...
for (j in ances){
  ...
  for (i in ses_factors){
    ...
    # Run models of interest
      formula = paste0("colorectal_incident ~ Deprivation_z + age_assessment + PSA_Screening"
                       ) %>% as.formula()
...
# And now export these models.
summ_df %>%
  write_csv( file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Draft_Figures/CRC/Figure2_models_2024.02.22.csv")
...
table2 %>% 
  write_csv(file = "/labs/jswitte/Projects/jjudd5/SES.PRS_Project/Draft_Figures/CRC/Table2_models_2024.02.22.csv")
